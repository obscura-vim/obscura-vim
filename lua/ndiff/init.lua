...
local function add_path(root, item)
	local current = root
	local parts = vim.split(item.name, "/", { plain = true, trimempty = true })
...
local function sorted_keys(table_value)
	local keys = vim.tbl_keys(table_value)
	table.sort(keys, function(left, right)
		local left_directory = next(table_value[left].children) ~= nil
		local right_directory = next(table_value[right].children) ~= nil
...
local function render_nodes(nodes, depth, lines, line_items)
...
local function create_tree(items)
	local root = {}
